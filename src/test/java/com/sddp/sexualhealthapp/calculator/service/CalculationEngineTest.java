...
class CalculationEngineTest {
    ...
    void testEvaluateExpression_SimpleAddition() {
        assertEquals(8.0, CalculationEngine.evaluateExpression("5+3"), DELTA);
    }

    @Test
    void testEvaluateExpression_SimpleSubtraction() {
        assertEquals(2.0, CalculationEngine.evaluateExpression("5-3"), DELTA);
    }

    @Test
    void testEvaluateExpression_SimpleMultiplication() {
        assertEquals(15.0, CalculationEngine.evaluateExpression("5×3"), DELTA);
        assertEquals(15.0, CalculationEngine.evaluateExpression("5*3"), DELTA);
    }

    @Test
    void testEvaluateExpression_SimpleDivision() {
        assertEquals(1.6667, CalculationEngine.evaluateExpression("5÷3"), DELTA);
        assertEquals(1.6667, CalculationEngine.evaluateExpression("5/3"), DELTA);
    }
    ...
    @Test
    void testEvaluateExpression_WithDecimals() {
        assertEquals(8.5, CalculationEngine.evaluateExpression("5.2+3.3"), DELTA);
    }
    ...
    void testEvaluateExpression_InvalidExpression() {
        assertThrows(IllegalArgumentException.class, () -> CalculationEngine.evaluateExpression("abc"));
        assertThrows(IllegalArgumentException.class, () -> CalculationEngine.evaluateExpression(""));
        assertThrows(IllegalArgumentException.class, () -> CalculationEngine.evaluateExpression("5+"));
    }

    @Test
    void testEvaluateExpression_DivisionByZero() {
        assertEquals(Double.POSITIVE_INFINITY, CalculationEngine.evaluateExpression("5÷0"));
    }

    @Test
    void testEvaluateExpression_RealWorldExamples() {
        // Common calculator use cases
        assertEquals(12.75, CalculationEngine.evaluateExpression("8.5+4.25"), DELTA);
        assertEquals(150.0, CalculationEngine.evaluateExpression("50×3"), DELTA);
        assertEquals(37.5, CalculationEngine.evaluateExpression("75÷2"), DELTA);
        assertEquals(85.0, CalculationEngine.evaluateExpression("100-15"), DELTA);
    }
}
